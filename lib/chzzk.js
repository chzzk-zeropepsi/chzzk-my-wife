...
(function () {
  ...
  // 로그인 사용자가 팔로우한 채널 ID 집합 (Set<string>). 로그인 쿠키 필요.
  async function getFollowings() {
    ...
    let total = Infinity;
    let guard = 0;
    while (page * size < total && guard < 50) {
      guard++;
      const url =
        "https://api.chzzk.naver.com/service/v1/channels/followings?page=" +
        page +
        "&size=" +
        size +
        "&sortType=FOLLOW";
      const res = await fetch(url, { credentials: "include" });
      if (!res.ok) throw new Error(`팔로잉 조회 실패: ${res.status}`);
      const data = await res.json();
      const c = (data && data.content) || {};
      const list = c.followingList || [];
      if (typeof c.totalCount === "number") total = c.totalCount;
      for (const it of list) {
        const id = it && (it.channelId || (it.channel && it.channel.channelId));
        if (id) ids.add(id);
      }
  ...
  globalThis.Chzzk = { getCurrentUser, searchChannels, getChannel, getFollowings };
})();
